	...
	<body class="horizontalmenu dark-theme">
						...
						<div class="row row-sm">
							<div class="col-lg-12">
								<div class="card custom-card overflow-hidden">
									<div class="card-body">
										<div>
											<h6 class="main-content-label mb-1">List transaction</h6>
										</div>
											...
											<table class="table" id="example1">
												<thead class="thead">
													<tr>
														...
														<th class="wd-20p">Price Total</th>
														<th class="wd-25p">Payment Method</th>
														<th class="wd-15p">Proof Of Payment</th>
													</tr>
												</thead>
												<tbody>
													<!--  -->
							...
							<div class="modal-dialog">
								...
								<div class="modal-content">
									<div class="modal-header">
										<h4 class="modal-title">Approval Transaction</h4>
									</div>
									<div class="modal-body">
										<form  method="POST" class="formproduk">
												<div class="form-group">
													<label for="judul">Transaction ID</label>
													<input type="hidden" name="idblog" id="idtransaction">
                                                    <input type="hidden" name="idblog" id="idproduk">
													<input type="text" name="judul" class="form-control" id="transactionid" readonly>
												</div>
												<div class="form-group">
                                                    <input type="hidden" name="idblog" id="status">
													<label for="isi">No Resi</label>
													<input type="text" name="author" class="form-control" id="noresi">
												</div>
												<div class="form-group">
                                                    <input type="hidden" name="idblog" id="status">
													<label for="isi">Product Name</label>
													<input type="text" name="author" class="form-control" id="productname" readonly>
												</div>
												<div class="form-group">
													<label for="author">Model Product Name</label>
                                                    <textarea name="isi" class="form-control" cols="10" rows="2" id="modelname" readonly='readonly'></textarea>
												</div>
                                                <div class="form-group">
													<label for="isi">Price Total</label>
													<input type="text" name="author" class="form-control" id="totalprice"readonly>
												</div>
												<div class="form-group">
													<label for="warna">Proof of Payment</label><br>
													<!-- <img src="#" alt="Foto tidak tersedia." id="foto" class="img-fluid mb-3" width="50%"> -->
                                                    <ul id="lightgallery" class="list-unstyled row mb-0 ml-1">
                                                        <li data-responsive="assets/img/media/1.jpg" data-src="assets/img/media/1.jpg" data-sub-html="">
                                                            <a href="">
                                                                <img src="assets/img/media/1.jpg" alt="Thumb-1" id="foto" class="img-fluid" width="60%">
                                                            </a>
                                                        </li>
                                                    </ul>
												</div>
		...
		<script>
			$(document).ready(function(){
                ...
                $('#example1').dataTable({
                    proccesiong: true,
                    ajax: 'function/view-approval.php',
                    order: [[0, 'desc']],
                    columns:[
                        {data: 'id_transaksi'},
                        {data: 'total_harga'},
                        ...
                        {
                            "render": function (data, type, JsonResultRow, meta) {
                                return '<img src="example/'+JsonResultRow.bukti_transfer+'" class="img-fluid">';
                            }
                        },
                    ],
                    'bDestroy': true
                ...
                $('#example1 tbody').on('click', 'tr', function () {
					...
					$("#idtransaction").val(id).trigger("change");
					$("#myModal").modal("show");
				});

                $('#idtransaction').bind('change', function(e){
                    e.preventDefault();
                    idtransaction = $(this).val();

                    $.ajax({
                        url: 'function/ajaxrespon-transaction.php',
                        type: 'GET',
                        dataType: 'json',
                        data: {
                        ...
                        success: function(data){
                            $('#idproduk').val(data['id_produk']).trigger('change');
                            $('#transactionid').val(data['id_transaksi']);
                            $('#totalprice').val(data['total_harga']);
                            $('#foto').attr('src', 'example/' + data['bukti_transfer']);
                            $('#status').val(data['status']).trigger('change');
                        }
                    });

                    $.ajax({
                        url: 'function/ajaxrespon-namamodel.php',
                        type: 'POST',
                        dataType: 'json',
                        data:{
                            'id_transaction': idtransaction
                    ...
                    $('#idproduk').bind('change', function(e){
                        ...
                        $.ajax({
                            url: 'function/ajaxrespon-namaproduk.php',
                            type: 'POST',
                            dataType: 'json',
                            data:{
                                'id_produk': idproduk
                            ...
                            success: function(data){
                                $('#productname').val(data['nama_produk']);
                            }
                        });
                    });
                    ...
                    $('li a').click(function(){
                        ...
                        $('li').attr("data-responsive", src);
                        // $('li').removeAttr('data-responsive');

                        $('li').attr("data-src", src);
                        // $('li').removeAttr('data-src');

                        $('li').attr('data-sub-html', filename);
                    });
                    ...
                    $("#status").bind('change', function(e){
                        ...
                        $('#approve').off().on('click', function(e){
                            ...
                            if(status == "2"){
                                ...
                                $.ajax({
                                    url: 'function/update-transaction.php',
                                    type: 'POST',
                                    cache: false,
                                    data:{
                                    ...
                                    success: function(dataResult){
                                        ...
                                        if(dataResult.statusCode==200){
                                            swal(
                                                "Well Done!",
                                                "You've been approved this transaction",
                                                "success"
                                            );
                                            $('#myModal').modal('hide');
						...
						$('#reject').off().on('click', function(e){
							e.preventDefault();
							...
							if(status == "1"){
                                ...
                                $.ajax({
                                    url: 'function/update-transaction-reject.php',
                                    type: 'POST',
                                    cache: false,
                                    data:{
                                        'idtransaction': idtransaction
                                    ...
                                    success: function(dataResult){
                                        ...
                                        if(dataResult.statusCode==200){
                                            swal(
                                                "Well Done!",
                                                "You've been rejected this transaction",
                                                "success"
                                            );
                                            $('#myModal').modal('hide');
                                            $('#example1').DataTable().ajax.reload(null, false);
