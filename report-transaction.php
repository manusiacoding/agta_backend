						...
						<div class="row row-sm">
							<div class="col-lg-12">
								<div class="card custom-card overflow-hidden">
									<div class="card-body">
										<div>
											<h6 class="main-content-label mb-3">List Transaction</h6>
										</div>
		...
		<script>
			$(document).ready(function(){
				$('#exportexample').dataTable({
					"ajax": "function/view-transaction.php",
					"order": [[0, "desc"]],
					"serverside": 'true',
					"processing": true,
					"dom": 'Bfrtip',
					...
					"columns": [
						...
						{ "render": function ( data, type, row ) {
                                var html = row.jumlah_produk + " Product"
                                return html;
                            }
                        },
						{ "render": function ( data, type, row ) {
                                var html = ""
                                if(row.shipping_courier_name == ''){
                                    html = ''
                                }else{
                                    html = row.shipping_courier_name
                                }
                                return html;
                            }
                        },
						...
						{ "render": function ( data, type, row ) {
                                var html = ""
                                if(row.status == '0'){
                                    html = 'Bukti pembayaran sedang divalidasi'
                                }else if(row.status == '1'){
                                    html = 'Bukti pembayaran tidak sesuai'
                                }else if(row.status == '2'){
                                    html = 'Pesanan dikirim'
                                }else if(row.status == '3'){
                                    html = 'Pesanan diterima'
                                }else if(row.status == '4'){
                                    html = 'Pesanan selesai'
                                }else if(row.status == '5'){
                                    html = 'Pesanan dibatalkan'
                                }
                                return html;
                            }
                        }
